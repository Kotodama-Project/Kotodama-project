...
EXPECTED_GATE_IDENTIFIERS = tuple(f"PB-G{index}" for index in range(1, 11))
...
class PublicBetaGateEnumerationTests(unittest.TestCase):
    ...
    def gate_lines(self) -> list[tuple[str, str, str]]:
        self.assertIn(GATE_SECTION, self.overview)
        section = self.overview.split(GATE_SECTION, 1)[1]
    ...
    def test_gate_identifiers_are_unique_and_sequential(self) -> None:
        ...
        self.assertEqual(identifiers, list(EXPECTED_GATE_IDENTIFIERS))
    ...
    def test_declared_count_matches_the_listed_gates(self) -> None:
        declared = DECLARED_COUNT.search(self.overview)
        self.assertIsNotNone(declared, "OVERVIEW must state the gate count explicitly")
