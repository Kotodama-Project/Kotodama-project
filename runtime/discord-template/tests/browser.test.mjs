...
test('real browser refuses coordinate Token reset, login and secret-area capture',{skip:!existsSync(executable),timeout:120000},async t=>{
  ...
  const browser=await chromium.launch({executablePath:executable,headless:true});t.after(async()=>{await browser.close();await new Promise(r=>server.close(r));});
  const warm=await browser.newPage();await warm.goto(base+'/warm-up');await warm.locator('#warm').click({timeout:60000});await warm.close();
  const page=await browser.newPage();const cli=new BrowserCli({allowedOrigins:[base]});cli.browser=browser;
