...
| 工程 | 自動で行うこと | 人が行うこと |
|---|---|---|
| PC/サーバー準備 | 依存関係、設定の雛形、診断、既存ブラウザへの接続準備 | 使用するサーバーと操作者を決める |
...
普段のブラウザですでにログインしている場合は、その有効なログインを保ちます。[既存セッションでの分担と任意のCLI接続](BROWSER.md)を確認し、対象のDeveloper Portalアカウントで次へ進みます。Lumaなどの連携先も、そのサービスへログイン済みのセッションを優先します。

ブラウザ全体への接続は必須ではありません。接続に不安がある場合は、本人が普段のブラウザで必要な設定を行い、CLIの診断やCSV取込へ戻れます。エージェントはアカウント指定だけを接続設定変更の許可へ読み替えません。

新しく独立した環境で作成する場合は、以下の専用ブラウザも利用できます。ログインが拒否されたら繰り返さず、普段のセッションへ戻ります。

![本人が操作するDiscordログイン画面。QRコードと個人情報の領域は含めていません。](images/discord-01-login.png)

```sh
node tools/browser-session.mjs --browser /path/to/chrome --url https://discord.com/developers/applications
```

表示されたloopback CDP URLをprivate設定の`browser.cdpUrl`へ入れ、`allowedOrigins`に`https://discord.com`を設定します。通常のブラウザのCookieをコピーしません。
