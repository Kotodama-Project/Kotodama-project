...
実行中の仕事は1秒ごとに、Taskの状態・設定・操作者と、出典チャンネルの閲覧権限を確認します。Discordへの権限確認はチャンネル単位にまとめ、結果を3秒だけ再利用します。チャンネル・ロール・スレッドなどの変更イベントを受けると再利用をやめ、次の確認で問い合わせ直します。権限なし、チャンネルや参加者の削除といった明確な拒否は、その確認で仕事を止めます。レート制限・Discord側の障害・通信エラーは「確認不能」として扱い、5秒以内かつ連続3回までは仕事を続けます。それを超えると従来どおり停止します。確認不能の間はログに `task_access` / `ACCESS_UNAVAILABLE` を記録します。
