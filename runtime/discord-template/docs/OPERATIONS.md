...
設定変更は現在の操作者・出典の検査に反映します。設定ファイルを読めない間は、誰も操作者・同意済み参加者として扱いません。ログには読めなくなった時に `policy_unavailable`、戻った時に `policy_restored` を一度ずつ記録します。音声モデルや接続先、実行先を変える場合は、このインストールを停止してから再起動します。
