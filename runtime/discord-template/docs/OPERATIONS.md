...
起動は`start`、確認は`status`、停止は`shutdown`です。バックグラウンドで動かす場合は、所有者・cwd・PID・起動時刻を持つサービス管理を使ってください。名前の一致だけで他のNodeやBotを止めません。停止の各段階には上限があります（Discordの切断15秒、音声記録20秒、仕事の実行15秒、制御・連携サーバー各5秒）。上限を超えた段階は `close_step_timeout` を記録して次へ進み、host lockを解放してから終了します。サービス管理の停止猶予は、この合計（最大60秒）より長くしてください（systemdの既定は90秒）。
