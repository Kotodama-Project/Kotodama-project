...
VoiceRoomの48kHz受信に接続済み。archiveを明示設定しvoice.storeAudioを有効にしたlocal ownerで使用する。journalPathはdataDir内の絶対path、archiveRootは既存保存先の絶対pathに限定し、保存用WhisperはローカルASRと同じhostに束縛する。既定では録音しない。
