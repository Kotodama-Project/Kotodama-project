...
export async function startRuntime(filename,{offline=false,analyzer,worker,runtimeDomain=process.env.KOTODAMA_RUNTIME_DOMAIN,log=value=>console.log(JSON.stringify(redact(value)))}={}){
  ...
  let pipeline;
  try{
    pipeline=new Pipeline({store,owner,config,analyzer:selectedAnalyzer,worker:worker??new CliWorker(config),authorize,authorizeAnalysis:task=>authorize(task,'analyze'),onTask:async task=>{if(discord)await discord.deliver(task);},onReply:async reply=>{if(discord)await discord.reply(reply);},onVoiceAction:async action=>{if(discord)await discord.voiceAction(action);},onError:code=>log({event:'operation_failed',code})});
    ...
    control=http.createServer(async(req,res)=>{
      ...
      try{
        ...
        if(req.method==='GET'&&req.url==='/v1/status'){send(200,{ownerId,pid:process.pid,startedAt,discord:discord?'connected':'offline_fixture',voice:voice?.control.status()??null,taskOwner:config.owner.kind,analysis:pipeline.admission.snapshot(),recovery:pipeline.recovery});return;}
  ...
  }catch(e){clearInterval(policyTimer);clearInterval(archiveTimer);await pipeline?.close();await discord?.close();await archive?.close();control?.close();bridge?.close();store.releaseHost(ownerId);store.close();throw e;}
