...
export function createWhisperArchiveAsr({sink,endpoint,authorize,language='ja',timeoutMs=120000,fetcher=fetch}){
  ...
  return async ({receipt,speakerId,source,binding:b})=>{
    ...
    const form=new FormData();form.append('audio',new Blob([audio],{type:'audio/mpeg'}),name);form.append('language',language);permitted(authorize,b,'asr');
    const signal=AbortSignal.timeout(timeoutMs);const raw=await responseJson(await fetcher(url,{method:'POST',body:form,redirect:'error',signal}),signal);
    permitted(authorize,b,'asr');check(Array.isArray(raw.segments)&&raw.segments.length<=4096&&Number.isFinite(raw.duration)&&raw.duration>=0&&typeof raw.language==='string','ARCHIVE_ASR_RESULT');
    return raw.segments.map((s,idx)=>{const confidence=s.avg_logprob===undefined?.8:Math.exp(s.avg_logprob);check(s.speaker_id===undefined||s.speaker_id===speakerId,'ARCHIVE_SPEAKER_MISMATCH');check(Number.isFinite(s.start)&&Number.isFinite(s.end)&&s.start>=0&&s.end>=s.start&&s.end<=raw.duration&&typeof s.text==='string'&&s.text.length<=10000&&Number.isFinite(confidence)&&confidence>=0&&confidence<=1,'ARCHIVE_ASR_SEGMENT');return {idx,start:s.start,end:s.end,text:s.text,confidence,...(speakerId?{speaker_id:speakerId}:{})};});
  };
}
