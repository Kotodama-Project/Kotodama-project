...
export class CliWorker {
  ...
  async run(task,context,{signal,authorize=async()=>{},onStart=()=>{}}={}) {
    ...
    if(write){check(cfg.worker.verify.length>0,'VERIFICATION_COMMAND_REQUIRED');await this.verifier.preflight({signal});}
    ...
    let cwd=cfg.worker.workspace,baseRevision=null,gitBinding=null;
    if(write){
      ...
      const head=await runCommand('git',['rev-parse','HEAD'],{cwd,signal,timeoutMs:15000});check(head.code===0,'WORKTREE_HEAD_MISSING');baseRevision=head.stdout.trim();gitBinding=digest(await readArtifact(path.join(cwd,'.git'),65536));
    }
    await authorize();
    ...
    if(write){
      check(digest(await readArtifact(path.join(cwd,'.git'),65536))===gitBinding,'WORKTREE_BINDING_CHANGED');
      const tracked=await runCommand('git',['-c','core.fsmonitor=false','diff','--no-ext-diff','--no-textconv','--name-only','--no-renames','-z',baseRevision],{cwd,signal,timeoutMs:30000});
      const untracked=await runCommand('git',['-c','core.fsmonitor=false','ls-files','--others','--exclude-standard','-z'],{cwd,signal,timeoutMs:30000});check(tracked.code===0&&untracked.code===0,'CHANGE_INVENTORY_FAILED');
      ...
      if(fresh.length){for(const name of fresh)await safePath(cwd,name);const staged=await runCommand('git',['-c','core.fsmonitor=false','--literal-pathspecs','add','--intent-to-add','--',...fresh],{cwd,signal,timeoutMs:30000});check(staged.code===0,'NEW_FILE_DIFF_FAILED');}
    ...
    if(write){const diff=await runCommand('git',['-c','core.fsmonitor=false','diff','--no-ext-diff','--no-textconv','--binary',baseRevision],{cwd,signal,timeoutMs:30000});check(diff.code===0,'DIFF_FAILED');const diffFile=path.join(resultRoot,'changes.patch');await writeFile(diffFile,diff.stdout,{encoding:'utf8',flag:'wx',mode:0o600});artifacts.push({path:diffFile,relative:'changes.patch',sha256:digest(diff.stdout),bytes:Buffer.byteLength(diff.stdout)});}
