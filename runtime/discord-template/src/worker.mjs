...
import {requireVerification,runVerification} from './verification.mjs';
export {readArtifact} from './artifact.mjs';
...
// Git inspection must not launch optional hooks, monitors or diff helpers.
const runGit=(args,options)=>runCommand('git',['-c','core.hooksPath=/dev/null','-c','core.fsmonitor=false',...args],options);
...
export class CliWorker {
  ...
  async run(task,context,{signal,authorize=async()=>{},onStart=()=>{}}={}) {
    ...
    if(write)await requireVerification(cfg.worker,{signal,onStart});
    ...
    if(write){
      ...
      const command=await runGit(['worktree','add','--detach',cwd,'HEAD'],{cwd:cfg.worker.workspace,signal,timeoutMs:30000});
      check(command.code===0,'WORKTREE_CREATE_FAILED');
      const head=await runGit(['rev-parse','HEAD'],{cwd,signal,timeoutMs:15000});check(head.code===0,'WORKTREE_HEAD_MISSING');baseRevision=head.stdout.trim();
    }
    await authorize();
    ...
    const answer=await invokeCodex(cfg.worker,{cwd,dataDir:cfg.dataDir,prompt,schema:resultSchema,sandbox:write?'workspace-write':'read-only',signal,onStart,decode,beforeFallback:async()=>{await authorize();if(write){const head=await runGit(['rev-parse','HEAD'],{cwd,signal,timeoutMs:15000});const status=await runGit(['status','--porcelain','--untracked-files=all','--ignored'],{cwd,signal,timeoutMs:15000});check(head.code===0&&head.stdout.trim()===baseRevision&&status.code===0&&!status.stdout.trim(),'FALLBACK_WORKSPACE_CHANGED');}}});
    ...
    if(write)for(const command of cfg.worker.verify){
      await authorize();const r=await runVerification(command,{cwd,signal,onStart,runtimeRoots:cfg.worker.verificationRuntimeRoots,timeoutMs:cfg.worker.timeoutSeconds*1000});
      validations.push({command:command.executable,exitCode:r.code,outputSha256:digest(r.stdout+r.stderr),sandbox:'bubblewrap',workspaceAccess:'read-only',network:'disabled'});
      if(r.code!==0)return {state:'failed',summary:'検証で失敗しました。変更候補を保持しています。',artifacts:[],validations};
    }
    const artifacts=[];let changed=[];
    if(write){
      const tracked=await runGit(['diff','--no-ext-diff','--no-textconv','--name-only','--no-renames','-z',baseRevision],{cwd,signal,timeoutMs:30000});
      const untracked=await runGit(['ls-files','--others','--exclude-standard','-z'],{cwd,signal,timeoutMs:30000});check(tracked.code===0&&untracked.code===0,'CHANGE_INVENTORY_FAILED');
      ...
      if(fresh.length){for(const name of fresh)await safePath(cwd,name);const staged=await runGit(['--literal-pathspecs','add','--intent-to-add','--',...fresh],{cwd,signal,timeoutMs:30000});check(staged.code===0,'NEW_FILE_DIFF_FAILED');}
    ...
    for(const relative of changed){
      ...
      if(!stat){const before=await runGit(['rev-parse',`${baseRevision}:${relative}`],{cwd,signal,timeoutMs:10000});check(before.code===0,'DELETED_FILE_BINDING_FAILED');artifacts.push({relative,deleted:true,priorBlobOid:before.stdout.trim()});continue;}
    ...
    if(write){const diff=await runGit(['diff','--no-ext-diff','--no-textconv','--binary',baseRevision],{cwd,signal,timeoutMs:30000});check(diff.code===0,'DIFF_FAILED');const diffFile=path.join(resultRoot,'changes.patch');await writeFile(diffFile,diff.stdout,{encoding:'utf8',flag:'wx',mode:0o600});artifacts.push({path:diffFile,relative:'changes.patch',sha256:digest(diff.stdout),bytes:Buffer.byteLength(diff.stdout)});}
