...
candidateが要求するdigestのimageが既にlocal Docker daemonへ存在するかは、[Compose Image Availability Preflight](../../docs/IMAGE-AVAILABILITY-PREFLIGHT.md)でread-only確認できます。imageがなければ拒否し、pullやstartへfallbackしません。
