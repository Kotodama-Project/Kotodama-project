...
- [x] [Public migration ledger](docs/PUBLIC-MIGRATION-LEDGER.md) with a hash-chained record schema, optional independently supplied trusted-head anchor, latest-subject disposition counts, separated terminal classification / transfer mechanism, deterministic verifier, negative tests, and a synthetic fixture. This is schema / verifier / test documentation only; the ledger file is not yet populated and no migration, copy, publication, Promotion, or Current Truth follows from it.
- [x] [Public agent lifecycle registry](docs/PUBLIC-AGENT-LIFECYCLE-REGISTRY.md) with a fail-closed run outcome contract, parent/child edges under zero-capable depth and fan-out budgets, unsuccessful-terminal-only retry, unique lease/event references, bounded input, a hash-chained append-only envelope, and a restart continuity assessment that never reaches verified. This is schema / verifier / test documentation only; no agent is spawned, no run is dispatched, and no Promotion or Current Truth follows from it.
