...
def render(config:dict,assets:Path):
    ...
    files={name:(assets/name).read_bytes() for name in ['install.sh','activate-instance.py','seal-check.py','verify-toolchain.py','kotodama-cloudflare-os.service']}
    plan={'schemaVersion':'kotodama.native-proxmox-template/v2','mode':'OFFICIAL_SOURCE_EVALUATION','coreRevision':CORE,'nodeVersion':NODE,'pnpmVersion':PNPM,'sourceRepository':'https://github.com/cloudflare/cloudflare-os','baseImageSha256':config['base_sha256'],'create':create,'seal':['pct','template',str(config['template_vmid'])],'clone':clone,'profile':config,'requiredTemplateState':{'accounts':False,'conversationData':False,'credentials':False},'templateStateVerified':False,'initialAgent':'unconfigured','productionSupport':'upstream-self-host-tooling-pending','assetHashes':{n:hashlib.sha256(b).hexdigest() for n,b in files.items()}}
