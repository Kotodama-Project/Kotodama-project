...
def valid_text(value: Any, minimum: int, maximum: int) -> bool:
    return (
        ...
        and SURROGATE_RE.search(value) is None
    )


def read_limited_bytes(path: Path) -> bytes | None:
    """Read at most one byte beyond the input contract."""
    try:
...
def valid_review_item(item: Any, category: str) -> bool:
    return (
        ...
        and valid_text(item["id"], 1, 512)
        and item["category"] == category
        and valid_item_path(item["path"])
        and valid_text(item["reason"], 1, 2000)
    )
...
def valid_request(request: Any) -> bool:
    ...
    return (
        ...
        and not set(review_paths).intersection(evidence_paths)
    )
