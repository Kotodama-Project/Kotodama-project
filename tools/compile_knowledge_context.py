...
def compile_context(workspace, *, ceiling="public", max_claims=8, max_bytes=16384, now=None):
    ...
    validation, package = validate_package(workspace, now)
    required_ids = set()
...
def main():
    ...
    try:
        result = compile_context(args.workspace, ceiling=args.ceiling, max_claims=args.max_claims, max_bytes=args.max_bytes, now=evaluation_time(args.as_of))
