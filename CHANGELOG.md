...
- BecomeOne から移植したアーキテクチャ文書（A022: 原則と境界、multi-agent の協調、plan runtime、supervision の契約）。再構成した 6 つの元ファイルの出典表（`migration/a022-public-architecture.provenance.json`）と、Issue #25 の owner 判断・非公開の元履歴走査 receipt・#18/#19 の基盤・main への PR での Dependency review・独立 review による受入の記録を付けた（[#29](https://github.com/Kotodama-Project/Kotodama-project/pull/29) を main に合わせて取り込み）。
