...
- 複数の人・AI の作業が同じリポジトリで重ならないようにする Git Steward の調整コア候補（書込み範囲の予約、lease と epoch による締め出し、独立した reviewer、SQLite の journal）を、元 PR [#57](https://github.com/Kotodama-Project/Kotodama-project/pull/57)・[#58](https://github.com/Kotodama-Project/Kotodama-project/pull/58) から main に当て直した（`runtime/git-steward/README.md`）。candidate-only で、Git・GitHub・provider への書込みと配備はしない。設計文書と業務演習は後続（[#132](https://github.com/Kotodama-Project/Kotodama-project/issues/132)）。
