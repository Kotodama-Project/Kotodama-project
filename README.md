...
[Checkpoint Segment Transition / Key Rotation Binding](docs/ATTESTATION-NONCE-STORE-CHECKPOINT-SEGMENT-TRANSITION.md)は、提示されたR20 bundle headから1つのsuccessor checkpointへの境界、同一store IDとappend-only reservation、旧/new signer policy、distinct reviewer policy、短時間window、pinned `ssh-keygen`を一つの署名済みcandidateとして検証します。key-rotation modeの成功は旧鍵失効、侵害不存在、canonical segmentation policy、complete history、実store continuityを証明しません。
