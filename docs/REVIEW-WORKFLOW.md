...
Intent抽出前のprivate Source Record shapeは[Company Pack Source Record Instance Contract](SOURCE-RECORD-INSTANCE.md)で確認できます。locator/hash/attribution/consent/retentionのschema PASSからsource authenticityやauthorityを導出しません。

Decisionへ渡す前のprivate Intent Candidate shapeは[Company Pack Intent Candidate Instance Contract](INTENT-CANDIDATE-INSTANCE.md)で確認できます。schema PASSやHuman入力欄からHuman Intent、source authenticity、consent、authorityを導出しません。
