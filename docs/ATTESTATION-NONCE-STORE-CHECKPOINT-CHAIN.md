...
生成済みself-contained bundle、checkpoint、signature、allowed-signers file、identity file、SQLite storeはすべてprivate operational dataです。公開repository、Issue、log、test corpusへ置かないでください。このrepositoryにはtool、schema、synthetic testだけを含めます。
...
## 2. Deterministic self-contained bundleを作る

bundle outputはchain directoryの外にある新規fileを指定します。既存fileは上書きしません。bundleはcheckpoint/signatureのexact bytesをcanonical Base64として内包するため、作成後のverifierはsource directoryを再openしません。

```powershell
python tools\create_attestation_nonce_store_checkpoint_chain_bundle.py `
  <private-chain-directory> `
  --output <private-chain-bundle.json>
```

builderは最大1,024 checkpoint、各checkpoint最大2 MiB、各signature最大64 KiB、raw chain aggregate最大16 MiB、encoded bundle最大24 MiBに制限します。bundleは連番locator、checkpoint/signature exact bytesとSHA-256、Genesis/current binding、ordered-chain digest、homogeneous signature policyを持ちます。作成時点ではsignatureを検証しないため、bundleの全authority claimはfalseです。

## 3. Protected Work Orderでpinする

material verificationの前に、少なくとも次をWork Orderへ固定します。

- private chain bundleとsupplied storeの対象
- self-contained bundle SHA-256の独立protected pin
...
`EXPECTED_BUNDLE_SHA256`は検証対象bundleから同じ手順内で自己計算した値ではなく、別personまたはprotected systemが保存した独立pinから渡します。CLIへdigestを渡すだけでは、そのpinの権威や不可変性は証明されません。

## 4. Chainとstoreを検証する

```powershell
python tools\verify_attestation_nonce_store_checkpoint_chain.py `
  <private-chain-bundle.json> `
...
- supplied self-contained bundleのexact SHA-256とclosed structureが一致する
- bundle内の全checkpoint/signature exact bytesが各digestと一致する
...
verifierはbundleを1個のdescriptorへ固定し、読込前にsizeを検査し、読込中のidentity/size/mtime driftを拒否します。source directoryは検証時に参照しないため、directory pathnameの途中差替えで別chainを混ぜられません。nonce storeは最大64 MiB、10,000 reservationで、queryには30秒のdeadlineがあります。最初に開いたstore descriptorを2回読み、同一digestのprivate immutable copyを作り、そのcopyと通常のSQLite read transactionのlogical snapshotを相互照合します。成功または失敗reportを確定するまでsource SQLite transactionとdescriptorを保持します。最大1,024 signature、全体120秒、1 signature 30秒の上限があります。長いchainはwriterを待たせ得るため、protected maintenance windowで実行してください。

`EXPECTED_SSH_KEYGEN_SHA256`もbundle pinと同じく、検証対象host上でその場計算した値ではなく独立protected pinから渡します。一致は実行bytesを固定しますが、そのbinaryのvendor provenance、supply-chain authority、脆弱性不存在までは証明しません。

## Exit codesとschema

- builder `0 / CHAIN_BUNDLE_CREATED`: deterministic self-contained private bundleを新規fileへ作成
