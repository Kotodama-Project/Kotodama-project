# Kotodama 設計と現在地の全文（Overview）

この文書は 2026-09-14 まで README だった全文です。製品の入口は [README.md](../README.md)、
状態の正本は [STATUS.md](../STATUS.md)、方向と現在地の対応は [PRODUCT-DIRECTION.md](PRODUCT-DIRECTION.md) です。

**会話を、監査できる意図・仕事・成果・学習へつなぐ Local-first Company OS。**

「楽しく過ごし、一緒に考え、必要なときだけ仕事を進める」は Voice channel で選べる利用モードの一つであり、
雑談を新しい実行依頼や追加の許可とみなさず、生産性へ誘導しない約束はそのモードで守られます。製品の本体は
Company OS（Evidence Chain、Company Pack、Context Platform、AI Workforce、Cloudflare edge と公式 Cloudflare OS 基盤）を
矛盾なく徹底して作ることで、人が一度「OK」と言った範囲では agent swarm が自律的に進める体験を最重要とします
（2026-09-14 の owner 補足。詳細は PRODUCT-DIRECTION.md）。既存の目的・許可がある読取調査、知識整理、改善提案、
ToDo 確認、継続作業は、新しい命令を毎回待たずに進められます。発話や推測を候補として整理することと、新しい仕事の
実行権限を得ることは別です。

公開のこのリポジトリを、製品統合・説明・導入の最優先の中心にします。Discord、Slack、OS、web は任意の入口です。
人も agent も、許可された同じ Source・Context・Knowledge・訂正・結果を参照し、人向け説明も同じ根拠から作ることを目指します。
...
作業を再開する方は **[プロジェクトの地図](PROJECT-MAP.md)** から、目標に対応する文書・実装・PR の関係を確認できます。エージェントの開始手順は [AGENTS.md](../AGENTS.md) にまとめています。
...
| **Try it** | 約5分の安全な体験と、次に選べる bounded path | [最初に選ぶ](#最初に選ぶ) / [5-minute tour](FIVE-MINUTE-TOUR.md) |
...
[Owner-confirmed Company AGI direction](OWNER-INTENT-COMPANY-AGI.md)。
...
機能ごとの到達点と現在の実行経路は **[README の実装状況](README-IMPLEMENTATION-ASSESSMENT.md)** を参照してください。[Task に結び付いた Company Pack 作成](COMPANY-PACK-TASK-EXECUTION.md) と [ローカルの確認・訂正 Gateway](../runtime/local-review-gateway/README.md) は実ファイル・実 HTTP の経路を持つ候補です。
...
まず全体の対応を一覧したい場合は [Company Pack Catalog](COMPANY-PACK-CATALOG.md) で確認してください。詳しい編集方法は [Template Guide](TEMPLATE-GUIDE.md)、最短の体験は [Starter Walkthrough](STARTER-WALKTHROUGH.md)、candidate-bound review は [Review Workflow](REVIEW-WORKFLOW.md) を参照してください。
...
と [Starter Walkthrough](STARTER-WALKTHROUGH.md) の説明に従って、候補側だけ
へ反映します。schema、validator、testの対応を一覧したい場合は
[Validation Guide](VALIDATION.md) を使います。
...
会社固有の値を反映した後は、[Company Pack Catalog](COMPANY-PACK-CATALOG.md)で
current candidateを再確認し、[Validation Guide](VALIDATION.md)と
[Review-chain artifact map](STARTER-WALKTHROUGH.md#review-chain-artifact-map)で
...
[content-free local runtime evaluation](CLOUDFLARE-OS-LOCAL-RUNTIME-EVALUATION.md)
...
- Human Decision 前の [Decision Record Candidate](DECISION-RECORD-CANDIDATE.md)、private な [Intent Candidate Instance](INTENT-CANDIDATE-INSTANCE.md)、[Source Record Instance](SOURCE-RECORD-INSTANCE.md) の closed schema 契約
- R31 record / Source Content / access evidence を照合する read-only [Source Binding Verification Candidate](SOURCE-BINDING-VERIFIER-CANDIDATE.md)
- 将来の protected runner receipt の field を固定する unpopulated [Protected Source Binding Receipt Candidate](PROTECTED-SOURCE-BINDING-RECEIPT-CANDIDATE.md)
...
未証明の gate はちょうど **10 件** です。各 gate には安定した ID を付けています。
[ROADMAP.md](../ROADMAP.md) の未完了項目はすべてこの ID を引用し、
[`tests/test_public_beta_gate_enumeration.py`](../tests/test_public_beta_gate_enumeration.py)
...
- `PB-G10` [unproven] 一つの実会話での Voice / Discord モードの受入（雑談だけの session は Task を作らない、出典付きの catch-up、既存の許可内での仕事、訂正の一本化）

`PB-G1` から `PB-G10` は安定した full registry です。gate が閉じても ID や行を削除・再採番せず、
`[unproven]` を `[completed]` に変えて revision-bound receipt を同じ行へ結びます。
新しい gate は末尾に次の番号で追加します。現在はすべて、公開面が Public Beta として証明されていない範囲そのものです。
...
| 目的 | 最初に読む | 次にできること |
|---|---|---|
| cloneから最短で安全な動作確認をする | [5-minute tour](FIVE-MINUTE-TOUR.md) | clone → 1 command → 結果を読むまでを外部接続なしで試す |
| Company Template、Blocks、Records、MOCsの関係だけを確認する | [Company Pack Catalog](COMPANY-PACK-CATALOG.md) | [Starter Walkthrough](STARTER-WALKTHROUGH.md)で理想/currentの差分を読む |
| starterを自分の候補へ複製し、localで検査する | [Starter Walkthrough](STARTER-WALKTHROUGH.md) | initializer、customization checker、validator、Review Bundleをread-only/candidate-onlyで実行する |
| runtime候補のprofileを選ぶ | [Installation Lifecycle](INSTALLATION-LIFECYCLE.md) | `compose_minimum`または`proxmox_segmented`を選び、preflightから始める |
...
[Starter Walkthrough](STARTER-WALKTHROUGH.md)には個別commandの説明、
...
Discordから使う最小構成の実装候補は[Discord runtime](DISCORD-RUNTIME.md)へ。
...
handoffは [Company Pack CLI Reference](COMPANY-PACK-CLI-REFERENCE.md) に
...
次に [Starter Walkthrough](STARTER-WALKTHROUGH.md) に沿って Human Intent reference、canonical owner、role、expiry、retention、profile を自分の候補へ置き換えます。

複数の確認を一度に再実行する場合は [Public Preview Self-check](PUBLIC-PREVIEW-SELF-CHECK.md) を使えます。既定のJSONは自動処理向け、`--format markdown`は同じ結果を人間が最初に読むための固定サマリーです。どちらも pack の構造、Catalog、customization の分類、false claim の境界だけを read-only で確認します。
...
`READY_FOR_GOVERNED_REVIEW` や bundle `MATCH` は、Human approval、execution authority、Promotion、Current Truth、Public Beta GO を意味しません。詳細は [Customization Checklist](CUSTOMIZATION-CHECKLIST.md)、[Review Bundle](REVIEW-BUNDLE.md)、[Review Workflow](REVIEW-WORKFLOW.md) を参照してください。
...
- [Compose Minimum Runbook](COMPOSE-MINIMUM-RUNBOOK.md)
- [Proxmox Segmented Runbook](PROXMOX-SEGMENTED-RUNBOOK.md)
- [Resolved Compose Candidate](RESOLVED-COMPOSE-CANDIDATE.md)
- [Image Availability Preflight](IMAGE-AVAILABILITY-PREFLIGHT.md)
- [Clean Install / Migration Evidence Candidate](CLEAN-INSTALL-MIGRATION-EVIDENCE-CANDIDATE.md)
...
- [Protected Compose Evidence Attestation](PROTECTED-COMPOSE-EVIDENCE-ATTESTATION.md)
- [One-Use Compose Attestation Evaluation](ONE-USE-COMPOSE-ATTESTATION-EVALUATION.md)
- [Attestation Nonce Store Checkpoint](ATTESTATION-NONCE-STORE-CHECKPOINT.md)
- [Attestation Nonce Store Checkpoint Chain](ATTESTATION-NONCE-STORE-CHECKPOINT-CHAIN.md)
...
個別の構造検査、negative case、JSON の安全な出力規則は [Validation Guide](VALIDATION.md) を参照してください。test PASS は、対象になった local bytes と契約の証拠です。Discord、provider、Proxmox、Docker daemon、本番 DB、実 Voice、Public Beta の E2E 証明へ拡張しません。
...
- [5-minute tour](FIVE-MINUTE-TOUR.md) — cloneから一行JSONの読み方までを約5分で確認する
- [Template Guide](TEMPLATE-GUIDE.md) — Blocks / Records / MOCs の編集
...
- [Company Pack Catalog](COMPANY-PACK-CATALOG.md) — current candidateの層と対応を一覧する
- [Starter Walkthrough](STARTER-WALKTHROUGH.md) — current read-only/candidate-onlyの最短体験
- [Public Preview Self-check](PUBLIC-PREVIEW-SELF-CHECK.md) — validator / Catalog / customization境界の一括read-only確認
- [Company Pack Guided Next Steps](COMPANY-PACK-NEXT-STEPS.md) — current stage、理想flow、分類別件数、次コマンドを案内する
- [Company Pack CLI Reference](COMPANY-PACK-CLI-REFERENCE.md) — 公開14 CLIの入力、状態、次handoff、共通helpを一覧する
- [Review-chain artifact map](STARTER-WALKTHROUGH.md#review-chain-artifact-map) — Review Bundle、Review Request、Review Response、Decision Handoffのartifact states and next handoffs
- [Installation Lifecycle](INSTALLATION-LIFECYCLE.md) — runtime profileを選ぶ前のread-only導線
...
- [Review Evidence to Decision Handoff](REVIEW-DECISION-HANDOFF.md)
- [Validation Guide](VALIDATION.md) — schema / validator / testの対応は[Schema / Validator / Test Matrix](SCHEMA-VALIDATOR-MATRIX.md)
- [Decision Record Candidate Contract](DECISION-RECORD-CANDIDATE.md)
- [Intent Candidate Instance Contract](INTENT-CANDIDATE-INSTANCE.md)
- [Source Record Instance Contract](SOURCE-RECORD-INSTANCE.md)
...
- [Protected Execution Request / Handoff Candidate](PROTECTED-EXECUTION-REQUEST-HANDOFF-CANDIDATE.md) — opaque request shape only; no runner or private evidence
...
Kotodamaがライセンスを設定できるコードは [MIT License](../LICENSE)（SPDX identifier: `MIT`）で提供します。第三者の条件と、既存移植部分の出典・表示は保持します。[適用範囲と未解決の出典確認](LICENSE-SCOPE.md)を確認してください。
