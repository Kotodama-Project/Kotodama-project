...
Genesisからcurrentまでの提示されたchain path全体とsupplied storeを検証する場合は、連番checkpoint/signature pairだけを含むprivate directoryからmanifestを作り、そのmanifestの独立protected pinをverifierへ渡します。manifest outputはchain directory外の新規fileにしてください。

```powershell
python tools\create_attestation_nonce_store_checkpoint_chain_bundle.py <private-chain-directory> --output <private-chain-manifest.json>
python tools\verify_attestation_nonce_store_checkpoint_chain.py <private-chain-manifest.json> <expected-manifest-sha256> <private-chain-directory> <supplied-store.sqlite3> <allowed-signers> <signer-identity-file> <expected-ssh-keygen-sha256>
```

`SIGNED_RECURSIVE_CHAIN_AND_STORE_EQUIVALENCE`はmanifest digest、全file digest、独立pinと一致する`ssh-keygen` exact bytesでの全OpenSSH signature、Genesis、全immediate-parent link、append-only reservation path、単一store ID、supplied store logical equivalenceだけを示します。最大1,024 checkpoint、aggregate 16 MiB、nonce store 64 MiB、store query 30秒、signature検証全体120秒です。verifierは全signature検証中もSQLite read transactionを保持するため、長いchainはmaintenance windowで実行します。成功しても`ssh-keygen`のvendor authority、external pinの権威、complete history、parallel branch不存在、key rotation、actual store continuity、backup作成、restore実行は証明しません。詳細は[Attestation Nonce Store Checkpoint Chain](ATTESTATION-NONCE-STORE-CHECKPOINT-CHAIN.md)を参照してください。
