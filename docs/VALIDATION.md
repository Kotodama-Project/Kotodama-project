...
python tools\verify_attestation_nonce_store_checkpoint_chain.py <private-chain-manifest.json> <expected-manifest-sha256> <private-chain-directory> <supplied-store.sqlite3> <allowed-signers> <signer-identity-file>
```

`SIGNED_RECURSIVE_CHAIN_AND_STORE_EQUIVALENCE`はmanifest digest、全file digest、全OpenSSH signature、Genesis、全immediate-parent link、append-only reservation path、単一store ID、supplied store logical equivalenceだけを示します。最大1,024 checkpoint、aggregate 16 MiB、signature検証全体120秒です。verifierは全signature検証中もSQLite read transactionを保持するため、長いchainはmaintenance windowで実行します。成功してもexternal pinの権威、complete history、parallel branch不存在、key rotation、actual store continuity、backup作成、restore実行は証明しません。詳細は[Attestation Nonce Store Checkpoint Chain](ATTESTATION-NONCE-STORE-CHECKPOINT-CHAIN.md)を参照してください。
