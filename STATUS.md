...
| [Public migration ledger](docs/PUBLIC-MIGRATION-LEDGER.md) | Included as an append-only schema, read-only verifier, tests, and a synthetic fixture; the ledger file itself is not yet populated, and migration execution, private continuity, publication, approval, Promotion, and Current Truth remain unverified |
| [Public agent lifecycle registry](docs/PUBLIC-AGENT-LIFECYCLE-REGISTRY.md) | Included as an append-only provider-neutral schema, read-only verifier, tests, and a synthetic fixture including a restart observation; agent spawn, dispatch, provider instance reuse, independent evidence verification, approval, Promotion, and Current Truth remain unverified |
